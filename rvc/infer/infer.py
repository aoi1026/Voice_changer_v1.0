...
class VoiceConverter:
    ...
    def convert_audio(
        ...
        **kwargs,
        ...
        try:
            ...
            output_path_format = wav_path.replace(
                ".wav", f".{export_format.lower()}"
            )
            audio_output_path = self.convert_audio_format(
                wav_path, output_path_format, export_format
            )
            # Keep only the selected format: remove intermediate WAV after successful export
            if (
                export_format.upper() != "WAV"
                and output_path_format
                and os.path.isfile(output_path_format)
                and wav_path
    ...
    def convert_audio_batch(
        ...
        **kwargs,
        ...
        Args:
            audio_input_paths: Either a folder path (str) or a list of input file paths.
            audio_output_path (str): Folder path where output files will be saved.
            **kwargs: Additional keyword arguments.
        """
        pid = os.getpid()
        try:
            ...
            if isinstance(audio_input_paths, (list, tuple)):
                # Explicit list of file paths (e.g. from bulk file selection)
                input_output_pairs = []
                for p in audio_input_paths:
                    if p and isinstance(p, str) and os.path.isfile(p):
                        base = os.path.splitext(os.path.basename(p))[0]
                        out_path = os.path.join(audio_output_path, base + "_output.wav")
                        input_output_pairs.append((p, out_path))
            else:
                # Folder path: discover all audio files in the folder
                audio_extensions = (
                ...
                input_output_pairs = [
                    (
                        os.path.join(audio_input_paths, a),
                        os.path.join(audio_output_path, os.path.splitext(a)[0] + "_output.wav"),
                    )
                    for a in audio_files
                ]

            print(f"Detected {len(input_output_pairs)} audio files for inference.")
            for new_input, new_output in input_output_pairs:
